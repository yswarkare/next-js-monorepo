const createObject = (length, createKeyValue) => {
	let result = {};
	for (let i = 1; i <= length; i++) {
		result = { ...result, ...createKeyValue(i) };
	}
	return result;
};

const flexSize = () => createObject(12, (i) => ({ [`${i}`]: `${i}` }));

const addPercentage = () => createObject(100, (i) => ({ [`${i}/100`]: `${i}%` }));

const createSize = () => createObject(165, (i) => ({ [`${i}`]: `${i * 0.1}rem` }));

const createGridTemplateColumns = () => createObject(12, (i) => ({ [`${i}`]: `repeat(${i}, minmax(0, 1fr))` }));

const gridSpan = () => createObject(12, (i) => ({ [`span-${i}`]: `span ${i} / span ${i}` }));

const spacing = () => {
	let counter = 1;
	let result = {};
	while (counter <= 100) {
		result = { ...result, [`${counter}`]: `${counter / 4}rem` };
		if (counter < 28) {
			counter++;
		} else if (counter >= 28 && counter < 64) {
			counter += 2;
		} else {
			counter += 4;
		}
